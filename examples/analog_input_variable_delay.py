...
async def analog_in(my_board, adc):
    ...
    # set the pin mode
    await my_board.set_pin_mode_analog_input(adc, differential=0, callback=the_callback)

    print('Enter Control-C to quit.')
    try:
        # await asyncio.sleep(5)
        print('Setting Scan Delay to 20ms for 3 seconds')
        await my_board.set_scan_delay(255)
        await asyncio.sleep(3)
        print('Setting Scan Delay to 2ms forever.')
        await my_board.set_scan_delay(2)
        while True:
            await asyncio.sleep(5)
